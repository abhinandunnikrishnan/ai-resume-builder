...
function ResumeInputs({resumeDetails,setResumeDetails}) {
  ...
  const renderFormContent=(stepCount)=>{
    switch (stepCount) {
      ...
      case 3: return(
        <div>
          <p>Our AI will generateSkills & Summary according to your job role.Once the form get submitted ,user will not gwt the chance to update the resume details.if you want to proceed CLick the <b>Generate AI Skills & Summary</b>button  </p>
        </div>
      )
        break;
  ...
  const handleSaveResume=async()=>{
    ...
    if(fullName && location && job && email && phone && github && linkedin && degree && college && year && skills.length>0 &&summary){
      ...
      if(response.status==201){
        toast.success("resume added successfuly")
    ...
    <Box sx={{ width: '100%' }}>
      ...
      {activeStep === steps.length ? (
        ...
        <React.Fragment>
          ...
          <Box>
            {/* render form content according to active step */}
            {
